...
def preparing_the_olympic_data_for_the_dot_plot(mini_df_team):
    ...
    # kinds of strokes: ['Breaststroke' 'Backstroke' 'Freestyle' 'Butterfly' 'Individual medley']
    list_of_strokes = mini_df_team['Stroke'].unique()
    for name_stroke in list_of_strokes:
        if name_stroke == 'Breaststroke':
            print('debug')
        ...
        line_values_for_worst = full_line_worst_score.loc[:,
        ...
        full_line_best_score = mini_df_by_stroke[
            mini_df_by_stroke['Results (In seconds)'] == mini_df_by_stroke['Results (In seconds)'].min()]
        line_values_for_best = full_line_best_score.loc[:,
                               ['Team', 'Stroke', 'Location', 'Year', 'Athlete', 'Results (In seconds)']]
        print('*')
        # improvement = (worst_score_ever_in_the_event - best_score_ever_in_the_event) / worst_score_ever_in_the_event * 100
        ...
        starting_point_for_the_team = (100 / (worst_score_ever_in_the_event - best_score_ever_in_the_event)) * (
                worst_score_ever_in_the_event - worst_score_achieved_by_the_team)
        ...
        ending_point_for_the_team = (100 / (worst_score_ever_in_the_event - best_score_ever_in_the_event)) * (
                worst_score_ever_in_the_event - best_score_achieved_by_the_team)
    ...
    print('*')
    final_table_fastest_results.rename(
        columns={final_table_fastest_results.columns[5]: 'Best results by the team (In seconds)'},
        inplace=True)  # 'Best results by the team (In seconds)'
    final_table_fastest_results['Best results by the team (In seconds)'] = final_table_fastest_results[
        'Best results by the team (In seconds)'].apply(lambda x: "{0:.2f}".format(x))
    ...
    final_table_slowest_results.rename(
        columns={final_table_slowest_results.columns[5]: 'Worst results by the team (In seconds)'},
        inplace=True)  # 'Worst results by the team (In seconds)'
    final_table_slowest_results['Worst results by the team (In seconds)'] = final_table_slowest_results[
        'Worst results by the team (In seconds)'].apply(lambda x: "{0:.2f}".format(x))
    ...
    #
    # # # Here below I am working on the table presentation using dataframe_image
...
def dot_plot_for_the_slowest_and_fastest_athletics(final_fastest_slowest_table):
    ...
    final_fastest_slowest_table["Improvement %"] = (final_fastest_slowest_table["Fastest_percentage"] - final_fastest_slowest_table["Slowest_percentage"]) / \
                                                   final_fastest_slowest_table["Fastest_percentage"]
    print('*')
    #plt.style.use('seaborn')
    plt.figure(figsize=(12, 6))
    y_range = np.arange(1, len(final_fastest_slowest_table.index) + 1)  # Dynamic Y range of axis
    colors = np.where(
        final_fastest_slowest_table['Fastest_percentage'] > final_fastest_slowest_table['Slowest_percentage'],
        '#d9d9d9', '#d57883')
    plt.hlines(y=y_range, xmin=final_fastest_slowest_table['Slowest_percentage'],
               xmax=final_fastest_slowest_table['Fastest_percentage'], color=colors, lw=10)
    plt.scatter(final_fastest_slowest_table['Slowest_percentage'], y_range, color='#0096d7', s=200,
                label='Less team improvement result', zorder=3)
    plt.scatter(final_fastest_slowest_table['Fastest_percentage'], y_range, color='#003953', s=200,
                label='Best team improvement result', zorder=3)  # 003953

    # Adding the annotation for the improvement
    ...
    fontdict_input_title = {'fontsize': 23, 'weight': 'heavy', 'alpha': 0.9, 'color': 'Navy',
                            'fontname': 'Franklin Gothic Medium Cond'}
    fontdict_input_y_axis = {'fontsize': 16, 'weight': 'heavy', 'alpha': 0.9, 'color': 'gray',
                             'fontname': 'Franklin Gothic Medium Cond'}
    ...
    plt.title(f"The improvement of the {team_iteration_str} team in each Olympic swimming events - Men's 200m",
              loc='left', fontdict=fontdict_input_title, pad=50)
    ...
    # set chart legend
    # plt.legend(labels = ["Best score by the team", "Worst score by the team"], loc=(0,1.07), ncol=2) # loc=(0,1.05), ncol=2)
    plt.legend(ncol=2, bbox_to_anchor=(1., 1.01), loc="lower right", frameon=False)
    # Adding x-axis title into the chart
    plt.text(55, 0.04, "Improvement by percentage - %", ha="center", va="center", weight='bold', style='italic',
             fontname='Franklin Gothic Medium Cond', fontsize=17)
...
if __name__ == '__main__':
    ...
    column_headers = list(
        df.columns.values)  # -> ['Location', 'Year', 'Distance (in meters)', 'Stroke', 'Relay?', 'Gender', 'Team', 'Athlete', 'Results', 'Rank']
    ...
    for team_names in list_of_teams:
        ...
        dot_plot_for_the_slowest_and_fastest_athletics(final_table)
    print('*')

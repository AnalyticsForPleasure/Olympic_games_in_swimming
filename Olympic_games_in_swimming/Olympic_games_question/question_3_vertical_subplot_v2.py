...
# **************************************************************************************************************
# Function  name: preparing_the_data_for_vertical_subplots
# input:
# return value: Converting  the values to seconds with two decimal place
# ****************************************************************************************************************
...
def plotting_subplot_for_freestyle_vs_medley_relay(df_each_year, men_df_year, women_df_year):
    ...
    men_medley = list(df_each_year.loc[(mini_df_year["Gender"] == 'Men') & (mini_df_year['Stroke'] == 'Medley')][
                          'Results (In seconds)'])  # ['Time_results'])
    women_medley = list(df_each_year.loc[(mini_df_year["Gender"] == 'Women') & (mini_df_year['Stroke'] == 'Medley')][
                            'Results (In seconds)'])  # ['Time_results'])
    men_Freestyle = list(df_each_year.loc[(mini_df_year["Gender"] == 'Men') & (mini_df_year['Stroke'] == 'Freestyle')][
                             'Results (In seconds)'])  # ['Time_results'])
    women_Freestyle = list(
        df_each_year.loc[(mini_df_year["Gender"] == 'Women') & (mini_df_year['Stroke'] == 'Freestyle')][
            'Results (In seconds)'])  # ['Time_results'])

    gender = ['Men', 'Women']
    styles_of_swimming = ['Freestyle',
                          'Medley']  # ['Men - Medley','Women - Medley','Men - Freestyle','Women - Freestyle']
    ...
    for swimmer_gender in gender:
        ...
        for subtitle_year, subtitle_location in zip(years_list, location_list):
            plt.title(f'{subtitle_location} - {subtitle_year} \nOlympic Games', fontweight="bold", loc='left',
                      fontsize=14, fontname='Franklin Gothic Medium Cond', x=-3.80, y=1, style='italic', color='navy')
...
if __name__ == '__main__':
    ...
    for olympic_year in years_list:
        ...
        plotting_subplot_for_freestyle_vs_medley_relay_v2(df_each_year, men_df_each_year, women_df_each_year)
        break
